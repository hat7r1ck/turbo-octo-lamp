...
The previous naming convention (`Exclusion List Review MM_YYYY_Week X`) used an arbitrary week 1 through 4 per month that did not always align with actual calendar weeks. Review scope was unclear because the same "Week 2" could mean different date ranges depending on who created the ticket.

This process uses the ISO 8601 week date standard (ISO 8601:2019) to eliminate that ambiguity. ISO 8601 assigns every week in the year a number from W01 through W52/W53. Weeks always start on Monday and end on Sunday. Week 01 is defined as the week containing the first Thursday of the year. This is the same standard the dashboard uses for `iso_week` in the review tracker.
...
The year and week number come directly from the ISO week shown on the dashboard. No month reference, no ambiguous week count. The scope of each ticket is exactly the entries displayed for that ISO week on the dashboard.
...
Use a Confluence Team Calendar to track which engineer is responsible for which review week. This gives the team a shared view organized by ISO 8601 week number so the rotation aligns with the dashboard.
...
**Approved:** The engineer inputs the approved exclusions into the KV store using the SCD Exclusion List Input dashboard, then closes the Jira ticket.
...
Once Strat approves the week, the assigned engineer inputs the approved exclusions into the live KV store through the SCD Exclusion List Input dashboard. This is a separate dashboard that writes directly to `WF02958.KV.IOC_Whitelist`. The review dashboard does not write to the KV store at any point. It only writes to the review tracker CSV.

1. Open the SCD Exclusion List Input dashboard
2. Input each approved exclusion per the SCD procedure
3. Verify the write using the query below
4. Close the Jira ticket for that review week once all approved entries are written

For the full procedure on updating the KV store, refer to the SCD Exclusion List Input documentation:

**[TODO: Link to SCD Exclusion List Input procedure document]**

Dashboard link: [SCD Exclusion List Input](https://es-wf-sp.splunkcloud.com/en-US/app/wf-ctfc-content/scd_exclusion_list_input)
